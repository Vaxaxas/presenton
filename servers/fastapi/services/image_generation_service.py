...
class ImageGenerationService:
    ...
    def get_image_gen_func(self):
        ...
        elif is_antigravity_image_selected():
            return self.generate_image_antigravity
        return None
    ...
    async def generate_image_antigravity(
        self, prompt: str, output_directory: str
    ) -> str:
        """Generate image using Antigravity OAuth credentials via Antigravity Gateway."""
                ...
                try:
                    async with session.post(
                        ...
                        if resp.status == 200:
                            ...
                            LOGGER.warning("Antigravity image generation failed on %s [%d]: %s", url, resp.status, text[:200])
                except Exception as exc:
                    LOGGER.warning("Antigravity image generation error on %s: %s", url, exc)

        if not data:
            raise HTTPException(
                status_code=500, detail="Antigravity image generation returned no response"
            )

        response = data.get("response", {})
        candidates = response.get("candidates", [])
        if not candidates:
            raise HTTPException(status_code=500, detail="No candidates returned from Antigravity image generation")
        ...
        if not image_path:
            raise HTTPException(
                status_code=500, detail="No image data returned from Antigravity model"
            )
